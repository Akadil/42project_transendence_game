...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleDisconnect(@ConnectedSocket() socket: Socket) {
        ...
        if (user) {
            ...
            if (
                ...
                this._waitingUser.handshake.query.token.toString() === token.toString()
            ) {
                this._waitingUser = null;
            }
            if (user.roomId !== '-1') {
                ...
                this.server
                    .to(room.roomId)
                    .emit('opponentLeft', { message: 'Opponent left the game' });
    ...
    matchMaking(@MessageBody() data: any, @ConnectedSocket() socket: Socket) {
        ...
        if (this._waitingUser === null) {
            ...
            try {
                let payloadOpponent = jwt.verify(
                    this._waitingUser.handshake.query.token.toString(),
                    process.env.JWT_SECRET
                ) as jwt.JwtPayload;
